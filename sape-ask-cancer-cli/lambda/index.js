...
class DictionaryAPIIntentControl extends AplControl {
  ...
  async handle(input, resultBuilder) {
    // Grab the phrase
    const keyword = input.request.intent.slots.Phrase.value; // Initialize the dictionary client    
...
class CancerDictionaryControl extends ContainerControl {
  constructor(props) {
    super(props);
    this.addChild(new LauncRequestControl('Welcome to CancerGov, the National Cancer Institute\'s skill. You can ask me to define a cancer-related term.', false)).addChild(new DictionaryAPIIntentControl('Input String', true, new AplDocument( /*#__PURE__*/React.createElement(HelloAplDocument, null)))).addChild(new HelpIntentControl('You can ask me to define a cancer term! How can I help?', false)).addChild(new StopOrCancelIntentControl('Goodbye!', true)).addChild(new IntentReflectorControl('IntentReflectorControl')).addChild(new SessionEndedRequestControl(null, false));
  }
